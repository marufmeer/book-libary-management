...
import { Borrow, BorrowModelType } from "../interfaces/borrow_interfaces";
import { Book } from "./book_model";

export const BorrowModel=new Schema<Borrow,BorrowModelType>({
    book: {
...
BorrowModel.post("save",async function(){
    ...
    try{
      ...
      await book.save()
      await BorrowM.updateAvailable(book._id)
    }
  catch(error){
...
export const BorrowM=model<Borrow,BorrowModelType>("Borrow",BorrowModel)